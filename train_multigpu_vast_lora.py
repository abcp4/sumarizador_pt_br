...
def parse_args() -> TrainConfig:
    ...
    parser.add_argument(
        "--lora-target-modules",
        type=str,
        default="q_proj,k_proj,v_proj,o_proj,wi_0,wi_1,wo",
        help="Lista separada por virgula de modulos alvo para LoRA.",
    )
    parser.add_argument(
...
def get_model_state_dict(model: torch.nn.Module) -> Dict[str, torch.Tensor]:
    if isinstance(model, DDP):
        return get_peft_model_state_dict(model.module)
...
def load_model_state_dict(model: torch.nn.Module, state_dict: Dict[str, torch.Tensor]) -> None:
    if isinstance(model, DDP):
        set_peft_model_state_dict(model.module, state_dict)
    else:
...
def save_checkpoint(
    ...
    state = {
        ...
        "adapter_state_dict": get_model_state_dict(model),
...
def load_checkpoint(
    ...
    adapter_state = checkpoint.get("adapter_state_dict") or checkpoint.get("model_state_dict")
    if adapter_state is None:
        raise ValueError("Checkpoint sem adapter_state_dict/model_state_dict para carregar LoRA.")
    load_model_state_dict(model, adapter_state)
...
def parse_lora_target_modules(target_modules_csv: str) -> List[str]:
    return [name.strip() for name in target_modules_csv.split(",") if name.strip()]
